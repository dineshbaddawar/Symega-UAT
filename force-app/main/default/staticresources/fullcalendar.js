		...
		$.each(
			{
				...
				mousedown: function(seg, ev) {
					...
					document.getElementById("title").innerHTML = seg.event.title;
					document.getElementById("recId").innerHTML = seg.event.id;
					document.getElementById("startDate").innerHTML = seg.event.start._d;
					//document.getElementById("address").innerHTML = seg.event.Job_Requisition__r.Street_Address__c;
					document.getElementById("endDate").innerHTML = seg.event.end._d;
					document.getElementById("startDateformat").innerHTML = seg.event.start._d.getTime();
					document.getElementById("endDateFormat").innerHTML = seg.event.end._d.getTime();
					document.getElementById("shiftName").innerHTML = seg.event.name;
					document.getElementById("sTime").innerHTML = moment(seg.event.start._d).format('MM/DD/YYYY'); 
					document.getElementById("eTime").innerHTML = moment(seg.event.end._d).format('MM/DD/YYYY'); 
					document.getElementById("shiftsTime").innerHTML = moment(seg.event.start._d).format("ddd, hA");
					document.getElementById("shifteTime").innerHTML = moment(seg.event.end._d).format("ddd, hA");
					var staffType = [];
					if(seg.event.Staff_Type__c != "" && seg.event.Staff_Type__c != undefined){
						var staffType = seg.event.Staff_Type__c.split(";");
						var result = "";
						staffType.forEach(function (item) {
							result += "<li>" + item;
						  });
						  document.getElementById('staffType').innerHTML = result;
					}
					/*if(seg.event.Computer_Skills__c != "" && seg.event.Computer_Skills__c != undefined){
						var cmpSkills = seg.event.Computer_Skills__c.split(";");
						var skills = "";
						cmpSkills.forEach(function (item) {
							skills += "<li>" + item;
						  });
						  document.getElementById('cmpSkills').innerHTML = result;
					} */
					if(seg.event.shiftAlreadyAllocated == true ||  seg.event.haveShiftOnSameDay == true ){
						  document.getElementById('appJobButton').style.display = "none";
						  if(seg.event.shiftAlreadyAllocated == true ){
							document.getElementById('withdrawJobButton').style.display = "block";
						}else{
							document.getElementById('withdrawJobButton').style.display = "none";
						}
					}else{
						document.getElementById('appJobButton').style.display = "block";
						document.getElementById('withdrawJobButton').style.display = "none";

					}
					
					debugger;
					if(seg.event.RequiredSkills.length > 0 && seg.event.RequiredSkills != undefined){
						var myList = document.getElementById('reqSkills');
						myList.innerHTML = '';
						var noOfSkill = seg.event.RequiredSkills.length;
						var skillToDisp = 0;
						for (var prop in seg.event.RequiredSkills) {
							
							if(skillToDisp < noOfSkill){
								document.getElementById('reqSkills').innerHTML += '<li>' + seg.event.RequiredSkills[prop] + '</li>';
								skillToDisp++;
							}
						}
					}
...
$.extend(TimeGrid.prototype, {
	...
	renderSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
		var isDraggable = false;//view.isEventDraggable(event);
