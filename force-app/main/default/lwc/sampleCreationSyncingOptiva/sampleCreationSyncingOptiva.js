...
import getProjRecList from '@salesforce/apex/Optiva_LWC_NPD_Controller.getProjectAndOppDet'; 
import updateUserRecord from '@salesforce/apex/Optiva_LWC_NPD_Controller.updateUser';
import updateAccRecord from '@salesforce/apex/Optiva_LWC_NPD_Controller.updateAccount';
import GetPicklistValues_Object from '@salesforce/apex/Optiva_LWC_NPD_Controller.GetPicklistValues_Object';
    ...
    submitProject(recTypeName){
        ...
        syncSample({recId:this.recordId,recType:recTypeName}).then(result=>{
            ...
            if(result){
                ...
                if(recTypeName == 'Tech Doc'){
                    if((this.responseStatus === 'Success')){
                        this.showToast('Success',this.responseMsg,'success');
                    }
                    else{
                        this.showToast('Failed',this.responseMsg,'warning');
                    }
                }
                else{
                    if((this.responseMsg == 'Customer creation in progress')){
                        this.showToast('Success',this.responseMsg,'success');
                    }
                    else if((result.status=='Success' && result.msgStatus=='S' && recTypeName == 'Project') || (result.status=='Success' && (recTypeName == 'Sample' || recTypeName == 'Application'))){
